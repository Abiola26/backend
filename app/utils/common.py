...
class DataProcessor:
    ...
    def process_analytics(records: List[FleetRecord]) -> AnalyticsResponse:
        ...
        for r in records:
            ...
            data.append(
                {"id": r.id, "date": r.date, "fleet": fleet_name, "amount": r.amount}
            )
        ...
        # 2. Daily Subtotals
        daily_grp = (
            df.groupby(["date", "fleet"])["amount"].agg(["sum", "count"]).reset_index()
        )
        ...
        record_objs = [
            FleetRecordOut(
                id=d["id"], date=d["date"], fleet=d["fleet"], amount=d["amount"]
            )
            for d in data
        ]
...
class ReportGenerator:
    ...
    def generate_excel(analytics: AnalyticsResponse) -> BytesIO:
        ...
        bus_summary_df[["REMITTANCE", "FUEL USED"]] = bus_summary_df.apply(
            calculate_metrics, axis=1
        )
            ...
            for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
                ...
                for cell in row:
                    if isinstance(cell.value, (int, float)):
                        ...
                        if (
                            "REVENUE" in header_str
                            or "REMITTANCE" in header_str
                            or "AMOUNT" in header_str
                            or "FUEL" in header_str
                        ):
    ...
    def generate_pdf(analytics: AnalyticsResponse) -> BytesIO:
        ...
        # Title
        elements.append(
            Paragraph("Fleet Reporting System - Analytical Report", styles["Title"])
        )
