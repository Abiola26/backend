...
def get_dashboard_stats(
    ...
    top_fleet_query = (
        base_query.with_entities(
            FleetRecord.fleet, func.sum(FleetRecord.amount).label("fleet_total")
        )
...
def get_analytics_charts(
    ...
    trend_query = (
        base_query.with_entities(
            FleetRecord.date, func.sum(FleetRecord.amount).label("total")
        )
        .group_by(FleetRecord.date)
        .order_by(FleetRecord.date)
    )

    revenue_trend = [
        ChartDataPoint(label=str(r.date), value=r.total) for r in trend_query.all()
    ]
    ...
    fleet_query = (
        base_query.with_entities(
            FleetRecord.fleet, func.sum(FleetRecord.amount).label("total")
        )
        .group_by(FleetRecord.fleet)
        .order_by(func.sum(FleetRecord.amount).desc())
    )
    ...
    revenue_by_fleet = [
        ChartDataPoint(label=r.fleet, value=r.total) for r in fleet_data
    ]
...
async def email_report(
    ...
    recipient = (
        email if email else current_user.username
    )  # Assuming username is email or valid
