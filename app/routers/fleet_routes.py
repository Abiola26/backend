...
def delete_records_batch(
    ...
    if start_date:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid start_date format (YYYY-MM-DD)"
            )
    ...
    if end_date:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid end_date format (YYYY-MM-DD)"
            )
...
def delete_record(
    ...
    crud.create_audit_log(
        db, current_user.id, current_user.username, "DELETE_RECORD", f"ID: {record_id}"
    )
