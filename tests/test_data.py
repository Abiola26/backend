...
def test_upload_excel(client, db):
    ...
    # Create admin via DB
    admin = User(
        username="admin", hashed_password=get_password_hash("password"), role="admin"
    )
    db.add(admin)
    db.commit()
...
def test_upload_invalid_format(client, db):
    # Create admin via DB
    admin = User(
        username="admin2", hashed_password=get_password_hash("password"), role="admin"
    )
    db.add(admin)
    db.commit()

    login_res = client.post(
        "/auth/token", data={"username": "admin2", "password": "password"}
    )
    token = login_res.json()["access_token"]
